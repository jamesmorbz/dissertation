...
def insert_state_message(conn, message):
    ...
    """,
        (
            ...
            payload["Wifi"]["RSSI"],
            payload["Wifi"]["Signal"],
            payload["Wifi"]["LinkCount"],
            payload["Wifi"]["Downtime"],
        ),
    )
    conn.commit()
...
def insert_sensor_message(conn, message):
    ...
    """,
        (
            message["topic"],
            payload["Time"],
            payload["ENERGY"]["TotalStartTime"],
            payload["ENERGY"]["Total"],
            payload["ENERGY"]["Yesterday"],
            payload["ENERGY"]["Today"],
            payload["ENERGY"]["Period"],
            payload["ENERGY"]["Power"],
            payload["ENERGY"]["ApparentPower"],
            payload["ENERGY"]["ReactivePower"],
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        return
    message = {"topic": msg.topic.split("/")[1], "payload": payload}
